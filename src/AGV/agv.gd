...
@export var home_transform: Transform3D = Transform3D.IDENTITY
...
func go_home_action() -> void:
	move_to_transform(_parent_xform() * home_transform)
...
func train_waypoint(waypoint_name: String) -> void:
	...
	waypoints[indexed_name] = transform
...
func go_to_waypoint(waypoint_name: String) -> void:
	if not waypoints.has(waypoint_name):
		...
		return
	var target: Transform3D = waypoints[waypoint_name]
	move_to_transform(_parent_xform() * target)
